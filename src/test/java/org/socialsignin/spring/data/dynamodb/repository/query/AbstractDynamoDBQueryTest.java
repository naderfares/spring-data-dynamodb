...
public class AbstractDynamoDBQueryTest {
  ...
  public void setUp() {
    ...
    doReturn(typeInformation).when(metadata).getReturnType(
        ArgumentMatchers.argThat(argument -> "findByName".equals(argument.getName())));
  ...
  public void testPaged() throws NoSuchMethodException, SecurityException {
    ...
    DynamoDBQueryMethod<User, String> dynamoDBQueryMethod =
        new DynamoDBQueryMethod<User, String>(method, metadata, factory);

    when(countQuery.getSingleResult()).thenReturn(total);
    when(query.getResultList()).thenReturn(content);

    TestAbstractDynamoDBQuery underTest =
        new TestAbstractDynamoDBQuery(dynamoDBOperations, dynamoDBQueryMethod);
