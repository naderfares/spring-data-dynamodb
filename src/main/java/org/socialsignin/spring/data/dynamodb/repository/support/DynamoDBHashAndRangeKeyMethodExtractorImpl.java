    ...
    ReflectionUtils.doWithMethods(idType, new MethodCallback() {
      ...
      public void doWith(Method method) {
        if (method.getAnnotation(DynamoDBHashKey.class) != null) {
          Assert.isNull(hashKeyMethod, "Multiple methods annotated by @DynamoDBHashKey within type "
              + idType.getName() + "!");
    ...
    ReflectionUtils.doWithFields(idType, new FieldCallback() {
      ...
      public void doWith(Field field) {
        if (field.getAnnotation(DynamoDBHashKey.class) != null) {
          Assert.isNull(hashKeyField, "Multiple fields annotated by @DynamoDBHashKey within type "
              + idType.getName() + "!");
    ...
    ReflectionUtils.doWithFields(idType, new FieldCallback() {
      ...
      public void doWith(Field field) {
        if (field.getAnnotation(DynamoDBRangeKey.class) != null) {
          Assert.isNull(rangeKeyField, "Multiple fields annotated by @DynamoDBRangeKey within type "
              + idType.getName() + "!");
          ReflectionUtils.makeAccessible(field);
          rangeKeyField = field;
        }
