 ...
 */
public abstract class AbstractDynamoDBQueryCriteria<T, ID>
    implements DynamoDBQueryCriteria<T, ID>, SortHandler {
  ...
  protected QueryConstants.ConsistentReadMode consistentReads =
      QueryConstants.ConsistentReadMode.DEFAULT;
  ...
  protected QueryRequest buildQueryRequest(String tableName, String theIndexName,
      String hashKeyAttributeName, String rangeKeyAttributeName, String rangeKeyPropertyName,
      List<Condition> hashKeyConditions, List<Condition> rangeKeyConditions) {
    ...
    if (isApplicableForGlobalSecondaryIndex()) {
      ...
      for (Entry<String, List<Condition>> singleAttributeConditions : attributeConditions
          .entrySet()) {
  ...
  protected void applySortIfSpecified(DynamoDBQueryExpression<T> queryExpression,
    ...
    if (permittedPropertyNames.size() > 1) {
      throw new UnsupportedOperationException(
          "Can only sort by at most a single range or index range key");
  ...
  protected void applySortIfSpecified(QueryRequest queryRequest,
      List<String> permittedPropertyNames) {
    if (permittedPropertyNames.size() > 2) {
      throw new UnsupportedOperationException(
          "Can only sort by at most a single global hash and range key");
    }
  ...
  private String getFirstDeclaredIndexNameForAttribute(
      Map<String, String[]> indexNamesByAttributeName, List<String> indexNamesToCheck,
  ...
  protected String getGlobalSecondaryIndexName() {
    ...
    if (globalSecondaryIndexName == null && attributeConditions != null
        && !attributeConditions.isEmpty()) {
      ...
      if (exactMatchIndexNames.size() > 1) {
        ...
        if (attributeConditions.size() == 1) {
          globalSecondaryIndexName =
              getFirstDeclaredIndexNameForAttribute(indexNamesByAttributeName,
  ...
  protected boolean hasIndexHashKeyEqualCondition() {
    ...
    if (hashKeyAttributeValue != null
        && entityInformation.isGlobalIndexHashKeyProperty(hashKeyPropertyName)) {
      hasIndexHashKeyEqualCondition = true;
    }
  ...
  protected boolean hasIndexRangeKeyCondition() {
    ...
    if (hashKeyAttributeValue != null
        && entityInformation.isGlobalIndexRangeKeyProperty(hashKeyPropertyName)) {
      hasIndexRangeKeyCondition = true;
    }
  ...
  public DynamoDBQueryCriteria<T, ID> withHashKeyEquals(Object value) {
    Assert.notNull(value,
        "Creating conditions on null hash keys not supported: please specify a value for '"
  ...
  @Override
  public DynamoDBQueryCriteria<T, ID> withPropertyBetween(String propertyName, Object value1,
      Object value2, Class<?> type) {
  ...
  public DynamoDBQueryCriteria<T, ID> withPropertyIn(String propertyName, Iterable<?> value,
    ...
    Condition condition =
        createCollectionCondition(propertyName, ComparisonOperator.IN, value, propertyType);
    return withCondition(propertyName, condition);
  }
  ...
  public DynamoDBQueryCriteria<T, ID> withSingleValueCriteria(String propertyName,
    ...
    if (comparisonOperator.equals(ComparisonOperator.EQ)) {
      ...
      Condition condition =
          createSingleValueCondition(propertyName, comparisonOperator, value, propertyType, false);
      return withCondition(propertyName, condition);
    }
  }
  ...
  protected abstract Query<Long> buildFinderCountQuery(DynamoDBOperations dynamoDBOperations,
      boolean pageQuery);
  ...
  protected <V extends Object> Object getPropertyAttributeValue(final String propertyName,
      final V value) {
    // TODO consider removing DynamoDBMarshaller code altogether as table model will
    // handle accordingly
    DynamoDBTypeConverter<Object, V> converter =
        (DynamoDBTypeConverter<Object, V>) entityInformation
  ...
  protected <V> Condition createNoValueCondition(String propertyName,
      ComparisonOperator comparisonOperator) {
  ...
  protected Condition createSingleValueCondition(String propertyName,
      ComparisonOperator comparisonOperator, Object o, Class<?> propertyType,
      boolean alreadyMarshalledIfRequired) {

    Assert.notNull(o,
        "Creating conditions on null property values not supported: please specify a value for '"
            + propertyName + "'");

    List<AttributeValue> attributeValueList = new ArrayList<>(1);
    Object attributeValue =
        !alreadyMarshalledIfRequired ? getPropertyAttributeValue(propertyName, o) : o;
  ...
  protected Condition createCollectionCondition(String propertyName,
      ComparisonOperator comparisonOperator, Iterable<?> o, Class<?> propertyType) {
  ...
  @Override
  public DynamoDBQueryCriteria<T, ID> withConsistentReads(
