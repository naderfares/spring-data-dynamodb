...
public abstract class AbstractDynamoDBConfiguration {
  ...
  /**
   * Return the base packages to scan for mapped {@link DynamoDBTable}s. Will return the package
   * name of the configuration class' (the concrete class, not this one here) by default. So if you
   * have a {@code com.acme.AppConfig} extending {@link AbstractDynamoDBConfiguration} the base
   * package will be considered {@code com.acme} unless the method is overriden to implement
   * alternate behaviour.
   *
   * @return the base package to scan for mapped {@link DynamoDBTable} classes or {@literal null} to
   *         not enable scanning for entities.
   */
  ...
  /**
   * Creates a {@link DynamoDBMappingContext} equipped with entity classes scanned from the mapping
   * base package.
   *
   ...
   *
   * @throws ClassNotFoundException if the class with {@link DynamoDBTable} annotation can't be
   *         loaded
   */
  @Bean
