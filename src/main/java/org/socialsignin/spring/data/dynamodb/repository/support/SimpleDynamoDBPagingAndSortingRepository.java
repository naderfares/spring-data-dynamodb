 ...
 * {@link org.springframework.data.repository.PagingAndSortingRepository} interface. Due to DynamoDB
 * limitations, sorting is not supported for find-all operations Due to DynamoDB limitations, paging
 * for find-all queries is not possible using an integer page number For paged requests, attempt to
 * approximate paging behavior by limiting the number of items which will be scanned, and by
 * returning a sublist of the result-set. NB: Number of results scanned for a given page request is
 * proportional to the page number requested!
 *
 ...
 */
public class SimpleDynamoDBPagingAndSortingRepository<T, ID> extends
    SimpleDynamoDBCrudRepository<T, ID> implements DynamoDBPagingAndSortingRepository<T, ID> {

  public SimpleDynamoDBPagingAndSortingRepository(
      DynamoDBEntityInformation<T, ID> entityInformation, DynamoDBOperations dynamoDBOperations,
  ...
  public Page<T> findAll(Pageable pageable) {
    ...
    assertScanEnabled(enableScanPermissions.isFindAllPaginatedScanEnabled(),
