...
public class DynamoDBTemplate implements DynamoDBOperations, ApplicationContextAware {
   ...
   * @param dynamoDBMapperConfig can be {@code null} - {@link DynamoDBMapperConfig#DEFAULT} is used
   *        if {@code null} is passed in
   * @param dynamoDBMapper can be {@code null} -
   *        {@link DynamoDBMapper#DynamoDBMapper(AmazonDynamoDB, DynamoDBMapperConfig)} is used if
  ...
  public <T> List<T> batchLoad(Map<Class<?>, List<KeyPair>> itemsToGet) {
    return dynamoDBMapper.batchLoad(itemsToGet).values().stream().flatMap(v -> v.stream())
        .map(e -> (T) e).map(entity -> {
          maybeEmitEvent(entity, AfterLoadEvent::new);
          return entity;
  ...
  public <T> PaginatedQueryList<T> query(Class<T> clazz, QueryRequest queryRequest) {
    ...
    return new PaginatedQueryList<T>(dynamoDBMapper, clazz, amazonDynamoDB, queryRequest,
        queryResult, dynamoDBMapperConfig.getPaginationLoadingStrategy(), dynamoDBMapperConfig);
  }
  ...
  public <T> String getOverriddenTableName(Class<T> domainClass, String tableName) {
    if (dynamoDBMapperConfig.getTableNameOverride() != null) {
      ...
      tableName = dynamoDBMapperConfig.getTableNameResolver().getTableName(domainClass,
