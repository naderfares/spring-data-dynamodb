...
public class DynamoDBQueryLookupStrategy {
  ...
  private abstract static class AbstractQueryLookupStrategy implements QueryLookupStrategy {
    ...
    public final RepositoryQuery resolveQuery(Method method, RepositoryMetadata metadata,
      ...
      return createDynamoDBQuery(method, metadata, factory, metadata.getDomainType(),
          metadata.getIdType(), namedQueries);
    }

    protected abstract <T, ID> RepositoryQuery createDynamoDBQuery(Method method,
        RepositoryMetadata metadata, ProjectionFactory factory, Class<T> entityClass,
        Class<ID> idClass, NamedQueries namedQueries);
  }
  ...
  private static class CreateQueryLookupStrategy extends AbstractQueryLookupStrategy {
    ...
    @Override
    protected <T, ID> RepositoryQuery createDynamoDBQuery(Method method,
        RepositoryMetadata metadata, ProjectionFactory factory, Class<T> entityClass,
        Class<ID> idClass, NamedQueries namedQueries) {
      try {
   ...
   * {@link org.springframework.data.jpa.repository.Query}. In case none is found we fall back on
   * query creation.
   *
   * @author Michael Lavelle
   * @author Sebastian Just
  ...
  private static class CreateIfNotFoundQueryLookupStrategy extends AbstractQueryLookupStrategy {
    ...
    protected <T, ID> RepositoryQuery createDynamoDBQuery(Method method,
        ...
        Class<ID> idClass, NamedQueries namedQueries) {
      try {
        return strategy.createDynamoDBQuery(method, metadata, factory, entityClass, idClass,
            namedQueries);
